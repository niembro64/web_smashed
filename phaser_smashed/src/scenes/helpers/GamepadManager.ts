...
export class GamepadManager {
  ...
  private initializeControllerMappings(): void {
    ...
    this.mappings.set('playstation', {
      ...
      buttons: {
        A: 1, B: 2, X: 0, Y: 3,  // PlayStation uses different face button layout
        L1: 4, R1: 5, L2: 6, R2: 7,
        select: 8, start: 9,
        L3: 10, R3: 11,
        up: 12, down: 13, left: 14, right: 15,
    ...
    this.mappings.set('mac-xbox', {
      ...
      buttons: {
        A: 0, B: 1, X: 3, Y: 4,  // Different mapping on Mac
        L1: 6, R1: 7, L2: 8, R2: 9,
        select: 10, start: 11,
        L3: 13, R3: 14,
    ...
    this.mappings.set('mac-playstation', {
      ...
      buttons: {
        A: 2, B: 1, X: 3, Y: 0,  // Different mapping on Mac
        L1: 4, R1: 5, L2: 6, R2: 7,
        select: 8, start: 9,
        L3: 10, R3: 11,
    ...
    this.mappings.set('switch-pro', {
      ...
      buttons: {
        A: 1, B: 0, X: 3, Y: 2,  // Nintendo button layout
        L1: 4, R1: 5, L2: 6, R2: 7,
        select: 8, start: 9,
        L3: 10, R3: 11,
        up: 12, down: 13, left: 14, right: 15,
    ...
    this.mappings.set('8bitdo', {
      ...
      buttons: {
        ...
        select: 10, start: 11,
        L3: 13, R3: 14,
        up: -1, down: -1, left: -1, right: -1  // Uses HAT
      },
      axes: { leftStickX: 0, leftStickY: 1, rightStickX: 2, rightStickY: 3 },
      dpadMode: 'hat',
      hatAxisIndex: 9
  ...
  private detectControllerType(gamepad: Gamepad): ControllerMapping {
    ...
    if (id.includes('playstation') || id.includes('dualshock') || id.includes('sony') || id.includes('wireless controller')) {
    ...
    if (id.includes('054c-05c4')) { // PS4 controller vendor/product ID
      return this.mappings.get(isMac ? 'mac-playstation' : 'playstation')!;
    }
    
    if (id.includes('045e-02d1') || id.includes('045e-02dd')) { // Xbox controller vendor/product IDs
      return this.mappings.get(isMac ? 'mac-xbox' : 'xbox')!;
    }
  ...
  private getHatDirection(hatValue: number): { up: boolean; down: boolean; left: boolean; right: boolean } {
    ...
    if (Math.abs(hatValue) < 0.1 || hatValue === null || hatValue === undefined) {
      return directions;
    }
  ...
  getGamepadState(index: number): any {
    ...
    if (mapping.buttons.A !== undefined && mapping.buttons.A >= 0 && mapping.buttons.A < gamepad.buttons.length && gamepad.buttons[mapping.buttons.A]) {
      state.A = gamepad.buttons[mapping.buttons.A].pressed;
    }
    if (mapping.buttons.B !== undefined && mapping.buttons.B >= 0 && mapping.buttons.B < gamepad.buttons.length && gamepad.buttons[mapping.buttons.B]) {
      state.B = gamepad.buttons[mapping.buttons.B].pressed;
    }
    if (mapping.buttons.X !== undefined && mapping.buttons.X >= 0 && mapping.buttons.X < gamepad.buttons.length && gamepad.buttons[mapping.buttons.X]) {
      state.X = gamepad.buttons[mapping.buttons.X].pressed;
    }
    if (mapping.buttons.Y !== undefined && mapping.buttons.Y >= 0 && mapping.buttons.Y < gamepad.buttons.length && gamepad.buttons[mapping.buttons.Y]) {
      state.Y = gamepad.buttons[mapping.buttons.Y].pressed;
    }
    
    if (mapping.buttons.L1 !== undefined && mapping.buttons.L1 >= 0 && mapping.buttons.L1 < gamepad.buttons.length && gamepad.buttons[mapping.buttons.L1]) {
      state.L = gamepad.buttons[mapping.buttons.L1].pressed;
    }
    if (mapping.buttons.R1 !== undefined && mapping.buttons.R1 >= 0 && mapping.buttons.R1 < gamepad.buttons.length && gamepad.buttons[mapping.buttons.R1]) {
      state.R = gamepad.buttons[mapping.buttons.R1].pressed;
    }
    
    if (mapping.buttons.start !== undefined && mapping.buttons.start >= 0 && mapping.buttons.start < gamepad.buttons.length && gamepad.buttons[mapping.buttons.start]) {
      state.start = gamepad.buttons[mapping.buttons.start].pressed;
    }
    if (mapping.buttons.select !== undefined && mapping.buttons.select >= 0 && mapping.buttons.select < gamepad.buttons.length && gamepad.buttons[mapping.buttons.select]) {
      state.select = gamepad.buttons[mapping.buttons.select].pressed;
    }
    
    // Handle D-pad based on mode
    if (mapping.dpadMode === 'buttons') {
      if (mapping.buttons.up !== undefined && mapping.buttons.up >= 0 && mapping.buttons.up < gamepad.buttons.length && gamepad.buttons[mapping.buttons.up]) {
        state.up = gamepad.buttons[mapping.buttons.up].pressed;
      }
      if (mapping.buttons.down !== undefined && mapping.buttons.down >= 0 && mapping.buttons.down < gamepad.buttons.length && gamepad.buttons[mapping.buttons.down]) {
        state.down = gamepad.buttons[mapping.buttons.down].pressed;
      }
      if (mapping.buttons.left !== undefined && mapping.buttons.left >= 0 && mapping.buttons.left < gamepad.buttons.length && gamepad.buttons[mapping.buttons.left]) {
        state.left = gamepad.buttons[mapping.buttons.left].pressed;
      }
      if (mapping.buttons.right !== undefined && mapping.buttons.right >= 0 && mapping.buttons.right < gamepad.buttons.length && gamepad.buttons[mapping.buttons.right]) {
        state.right = gamepad.buttons[mapping.buttons.right].pressed;
      }
    } else if (mapping.dpadMode === 'hat' && mapping.hatAxisIndex !== undefined && mapping.hatAxisIndex < gamepad.axes.length) {
    ...
    if (mapping.axes.leftStickX !== undefined && mapping.axes.leftStickX < gamepad.axes.length) {
      state.leftStickX = this.applyDeadZone(gamepad.axes[mapping.axes.leftStickX]);
    }
    if (mapping.axes.leftStickY !== undefined && mapping.axes.leftStickY < gamepad.axes.length) {
      state.leftStickY = this.applyDeadZone(gamepad.axes[mapping.axes.leftStickY]);
    }
    if (mapping.axes.rightStickX !== undefined && mapping.axes.rightStickX < gamepad.axes.length) {
      state.rightStickX = this.applyDeadZone(gamepad.axes[mapping.axes.rightStickX]);
    }
    if (mapping.axes.rightStickY !== undefined && mapping.axes.rightStickY < gamepad.axes.length) {
      state.rightStickY = this.applyDeadZone(gamepad.axes[mapping.axes.rightStickY]);
    }
  ...
  isRealController(gamepad: Gamepad): boolean {
    ...
    const nonGamingDevices = ['jabra', 'headset', 'audio', 'microphone', 'webcam'];
